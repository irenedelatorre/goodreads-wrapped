...
wait_until_logged_in <- function(timeout = 300, poll = 3) {
  t0 <- Sys.time()
  repeat {
    # try going to My Books (if you’re not logged in, it redirects to sign_in)
    ...
    if (!inherits(curr, "try-error")) {
      # if it does NOT contain "sign_in", you’re probably logged in
...
# It’s a good idea to wrap with `possibly()` in case any page fails.
...
# 📉 15 seconds/book
# 📉 From 200 books = 50 seconds → 2–10 minutes
